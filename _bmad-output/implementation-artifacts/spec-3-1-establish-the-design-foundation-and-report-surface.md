...
context: ['_bmad-output/planning-artifacts/ux-designs/ux-darkhouse-2026-08-12/DESIGN.md']
---
...
**Never:**
- No route, persistence, navigation, or API call in 3.1; no scanner/`contracts` changes; no schema validation (deferred `@darkhouse/contracts`).
...
- `apps/web/components/audience-toggle.tsx` (new) -- segmented Client/Developer control; `role="group"` + two buttons with `aria-pressed`; restore-from-`sessionStorage` in `useEffect` under key `darkhouse:audience` (default Client; avoids hydration mismatch).
...
- [x] [Review][Patch] P-2 `metadata.title` / description are still Create-Next-App scaffold text — `apps/web/app/layout.tsx:15-18` — an accessibility product shipping "Create Next App" as its document title is an a11y miss. **Fixed: real Darkhouse title + description.**
...
- [x] [Review][Defer] W-2 The storage key `darkhouse:audience` is global across all future ReportSurfaces in a session — deferred, pre-existing at wiring time.
...
- **Verification (spec commands):** `pnpm --filter @darkhouse/web test` → 118/118 across 10 files; `pnpm --filter @darkhouse/web lint` → clean; `pnpm --filter @darkhouse/web build` → succeeds under Next 16 Turbopack (Inter/JetBrains Mono font swap + new client components compile).
...
**Commands:**
- `pnpm --filter @darkhouse/web test` -- expected: new suites green alongside existing ~100 tests.
- `pnpm --filter @darkhouse/web lint` -- expected: clean.
- `pnpm --filter @darkhouse/web build` -- expected: succeeds (font swap + new client components compile).
